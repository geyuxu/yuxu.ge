...
// ============================================================
const CONFIG = {
    workerUrl: 'https://yuxu.ge/api/embedding',
...
export class SearchClient {
    ...
    async init() {
        try {
            ...
            const [voyModule, indexResponse, metadataResponse, invertedResponse] = await Promise.all([
                import('/public/lib/voy-loader.js').then(m => m.getVoy()),
                fetch(this.indexUrl),
                fetch(this.metadataUrl),
                fetch(this.invertedIndexUrl),
            ]);

            // Load Voy index
            if (indexResponse.ok) {
                const indexData = await indexResponse.text();
                this.voy = voyModule.deserialize(indexData);
    ...
    async semanticSearch(query, limit = this.topK * 2) {
        ...
        try {
            const embedding = await getVector(query.trim());

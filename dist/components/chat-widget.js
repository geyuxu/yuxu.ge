...
// Load config from sidebar-config.json
async function loadChatConfig() {
    try {
        const response = await fetch('/sidebar-config.json');
        const config = await response.json();
        if (config.chatApi) {
            CHAT_CONFIG.apiUrl = config.chatApi;
        }
    } catch (e) {
        console.log('[Chat] Using default config');
    }
}
...
export async function initChatWidget() {
    ...
    try {
        ...
        // Load sidebar config for search settings
        let sidebarConfig = {};
        try {
            const configRes = await fetch('/sidebar-config.json');
            sidebarConfig = await configRes.json();
        } catch (e) {}
